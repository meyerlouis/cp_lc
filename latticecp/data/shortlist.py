...
ALIAS_DROP = {
    "parity5_plus_5", "online-shoppers-intention", "blastchar", "kr-vs-k",
    "thyroid-allrep", "thyroid-allbp", "thyroid-dis",
}
...
def _round_robin(group: pd.DataFrame, config: ShortlistConfig) -> pd.DataFrame:
    ...
    bins = [rows.sort_values("n_per_cell", ascending=False)
...
def make_shortlist(scan: pd.DataFrame,
                   config: ShortlistConfig = ShortlistConfig()) -> pd.DataFrame:
    ...
    d = d.drop_duplicates(subset=["dataset", "combo", "max_k"]
                          if "max_k" in d else ["dataset", "combo"])
    d = d[(d.lattice >= 2) & (d.lattice <= config.lattice_max)
    ...
    shortlist = (pd.concat(parts, ignore_index=True) if parts
                 else d.iloc[:0].copy())
